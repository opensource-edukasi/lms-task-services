...
// createPost calls lms-post-service to create a post with type TASK (3)
func (c *postServiceClient) createPost(ctx context.Context, subjectClassID, taskID, title, userID string) {
	...
	// Field 4: type (int32) = 3 (TASK)
	// Field 5: type_id (string) = taskID
	...
	buf = protowire.AppendVarint(buf, 3) // TASK = 3
	buf = protowire.AppendTag(buf, 5, protowire.BytesType)
	buf = protowire.AppendString(buf, taskID)
	...
	if err != nil {
		c.Log.Printf("error creating post for task: %v", err)
		return
	}

	c.Log.Printf("successfully created post for task %s", taskID)
}
